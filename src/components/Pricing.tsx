        ...
        {(["24", "12", "monthly"] as PlanId[]).map((plan) => (
          <button
            ...
            className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex flex-col items-center gap-1 transition-all duration-200
              ${activePlan === plan ? "tab-active text-white" : "text-slate-400 hover:text-white"}`}
          ...
          >
            ...
            <span
              className={`text-[10px] px-2 py-0.5 rounded-full border
                ${plan === "24" ? "bg-indigo-600/30 text-indigo-400 border-indigo-500/20" : 
                  plan === "12" ? "bg-emerald-600/30 text-emerald-400 border-emerald-500/20" : 
      ...
      >
        ...
        <div className="flex-1 p-8 lg:p-12">
          ...
          <div className="flex items-baseline gap-3">
            ...
            {current.old && <span className="text-xl text-slate-500 line-through">{current.old}</span>}
          </div>
          <p className="text-slate-400 mt-2 text-sm">Access for {activePlan === "24" ? "24 months" : activePlan === "12" ? "12 months" : "1 month"}. Pay once, no auto-renewal.</p>
